...
def test_validate_password(user_service):
    ...
    assert user_service.validate_password('short') == (False, "Password must be at least 8 characters")
    assert user_service.validate_password('nouppercase1!') == (False, "Password must contain uppercase letter")
    assert user_service.validate_password('NOLOWERCASE1!') == (False, "Password must contain lowercase letter")
    assert user_service.validate_password('NoDigit!') == (False, "Password must contain a digit")
    assert user_service.validate_password('NoSpecialChar1') == (False, "Password must contain a special character")
...
def test_authenticate(user_service, user_data):
    ...
    assert not user_service.authenticate(user_data['email'], 'wrongpassword')
    assert not user_service.authenticate('wrongemail@example.com', user_data['password'])
...
def test_user_crud_operations(user_service, user_data):
    ...
    # Update user
    assert user_service.update_user(user_data['email'], name='Updated Name')
    updated_user = user_service.get_user(user_data['email'])
    assert updated_user['name'] == 'Updated Name'

    # Delete user
    assert user_service.delete_user(user_data['email'])
    assert not user_service.get_user(user_data['email'])
